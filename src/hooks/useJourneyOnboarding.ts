...
import type { OnboardingQuestion } from "@/src/components/journey/JourneyOnboardingScreen";
...
const QUIZ_QUESTIONS: OnboardingQuestion[] = [
  {
    ...
    subtitle:
      "Pick the one that resonates most — you can always explore more later.",
    options: [
      {
        id: "calm_anxiety",
    ...
    if (matches.length > 0) {
      ...
      const diffMatch: MentalHealthJourneyListItem | undefined = matches.find(
        (j: MentalHealthJourneyListItem) =>
          j.difficulty === difficultyPreference,
      );
