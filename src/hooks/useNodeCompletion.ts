...
function checkSectionComplete(
  ...
  for (let i = 0; i < template.units.length; i++) {
    ...
    const lastNode: MentalHealthTemplateNode | undefined =
      unit.nodes[unit.nodes.length - 1];
...
export function useNodeCompletion(
  ...
  const [lastResult, setLastResult] = useState<NodeCompletionResult | null>(
    null,
  );
      ...
      try {
        ...
        if (!logRes.success) {
          const result = errorResult(
            logRes.error ?? "Failed to log completion",
          );
        ...
        if (!completeRes.success) {
          console.warn(
            "[useNodeCompletion] RPC failed, continuing with local update:",
            completeRes.error,
          );
        }
        ...
        // ── Step 7: Optimistic Redux state update ──
        dispatch(completeNodeAction(params.nodeId));
