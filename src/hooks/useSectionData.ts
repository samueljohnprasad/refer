...
function summarizeProgressRows(
  progress: SectionMapResponse["progress"],
): Array<{
  nodeId: string;
  status: string;
  progress: number;
...
export function useSectionData(
            ...
            if (isOnline && !isFresh) {
              ...
              if (freshRes.success && freshRes.data) {
                ...
                log.info("Refreshed in-memory section cache", {
                  ...
                  progressRows: summarizeProgressRows(freshRes.data.progress),
                });
              }
            }
            return;
  ...
  const loadSection = useCallback(
    async (unitNumber: number): Promise<void> => {
      ...
      // Check in-memory cache synchronously before showing skeleton
      const cacheKey: string = `${viewMode}:${unitNumber}`;
      const cached: SectionMapResponse | undefined =
        inMemoryCache.get(cacheKey);
      if (cached) {
        // Instant render from cache — no skeleton needed
        setSectionMap(cached);
        // Still fetch in background (via fetchAndApplySection) without skeleton
        fetchAndApplySection(slug, unitNumber).catch(() => {});
        return;
      }
    ...
    [slug, fetchAndApplySection, viewMode, inMemoryCache, setSectionMap],
  );
