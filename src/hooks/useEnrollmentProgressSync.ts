...
export function useEnrollmentProgressSync(): void {
  ...
  const enrollments: JourneyEnrollment[] = useAtomValue(journeyEnrollmentsAtom);
  const setEnrollments = useSetAtom(journeyEnrollmentsAtom);

  // Keep a ref of enrollments so the effect can read the latest value
  // without including it in the dependency array (which would cause loops).
  const enrollmentsRef = useRef<JourneyEnrollment[]>(enrollments);
  ...
  useEffect(() => {
    ...
    // Read from ref to avoid dependency loop
    const currentEnrollments: JourneyEnrollment[] = enrollmentsRef.current;

    // Check if this enrollment's progress actually differs
    const currentEnrollment: JourneyEnrollment | undefined =
      currentEnrollments.find((e: JourneyEnrollment) => e.slug === activeSlug);
    if (
      currentEnrollment &&
    ...
    const updatedEnrollments: JourneyEnrollment[] = currentEnrollments.map(
  ...
  }, [signature, activeSlug, setEnrollments]);
}
