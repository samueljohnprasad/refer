...
export async function fetchMHJourneyTemplate(
  ...
  try {
    ...
    const toUnit = (unit: Record<string, unknown>): MHTemplateUnit => ({
      ...
      globalUnitNumber: (unit.globalUnitNumber ?? unit.unitNumber ?? null) as
        | number
        | undefined,
      ...
      mascotPlacements: (unit.mascotPlacements ??
    ...
    const sections: MHTemplateSection[] = (
      ...
      mascotPlacements: (section.mascotPlacements ??
        []) as MHTemplateSection["mascotPlacements"],
      units: ((section.units ?? []) as Record<string, unknown>[]).map(toUnit),
    }));

    const flatUnits: MHTemplateUnit[] =
      sections.length > 0
...
/**
 * Fetch journeys the user is enrolled in.
 * Calls the Supabase edge function to query user_journey_enrollments and journey_templates.
 */
export async function fetchEnrolledJourneys(): Promise<
  ApiResponse<{
...
> {
  try {
    ...
    const response = await fetch(
      `${supabaseUrl}/functions/v1/fetch-enrolled-journeys`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      log.error("fetchEnrolledJourneys edge function error", errorText);
      return {
        data: { items: [], activeSlug: null },
        success: false,
        error: errorText || "Edge function request failed",
      };
    }
    ...
    return {
      data: {
        items: result.data as MentalHealthJourneyListItem[],
        activeSlug: result.activeSlug as string | null,
      },
      success: true,
    };
  } catch (err) {
    log.error("fetchEnrolledJourneys exception", err);
    return {
      data: { items: [], activeSlug: null },
      success: false,
      error: err instanceof Error ? err.message : "Unknown error",
