...
export function CommonTrapCategoryEngine({
  ...
  // ponytail: engine delegates primary transitions to the wrapper via config.ts
  useEffect(() => {
    if (!saved || !saved.phase) {
      onInteraction(
        ...
        true, // always ready so wrapper footer button is enabled
      );
    } else {
      // Ensure we remain ready if restored from cache in an intermediate state
      onInteraction(
        {
          format: CourseExerciseCategoryEnum.CommonTrap,
          phase,
