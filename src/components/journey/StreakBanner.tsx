...
function StreakBannerInner({
    ...
    if (compact) {
        return (
            ...
            >
                ...
                <Text
                    className={`text-sm font-bold ${isAtRisk ? 'text-orange-500' : 'text-orange-600'
                        }`}
                >
                    {currentStreak}
                </Text>
    ...
    return (
        <Pressable
            onPress={onPress}
            className={`flex-row items-center px-3 py-2 rounded-2xl border ${isAtRisk
        ...
        >
            ...
            <View className="ml-1.5 mr-2">
                {hasStreak ? (
                    <View className="flex-row items-baseline gap-1">
                        <Text
                            className={`text-base font-bold ${isAtRisk ? 'text-orange-600' : 'text-amber-700'
                                }`}
                        >
                            {currentStreak}
                        </Text>
            ...
            {/* At risk indicator */}
            {isAtRisk && hasStreak ? (
                <View className="bg-orange-100 px-1.5 py-0.5 rounded-full mr-1">
                    <Text className="text-xs font-bold text-orange-600">⚠️</Text>
                </View>
            ) : null}
