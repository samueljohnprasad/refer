# Matching Exercise Sequential Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign the Matching Exercise UI to use a sequential layout where one concept is presented at a time to reduce cognitive load.

**Architecture:** We will replace the unified grid layout with a sequential flow. State will track unmatched concepts and options separately. Interaction handlers will validate matches, provide brief visual feedback, and auto-progress to the next concept.

**Tech Stack:** React Native, NativeWind, React Hooks.

## Global Constraints

- Must use existing UI components (`Text`, `Card`, `Mascot`).
- Code must be written in TypeScript with proper typing.
- Layout must be mobile-friendly and accessible.
- The `isReady` flag must only be true when all pairs have been matched.
- Do not use inline styles; use Tailwind CSS classes.

---

### Task 1: Update State Management for Sequential Flow

**Files:**
- Modify: `src/components/exercise/MatchingExercise.tsx:1-94`

**Interfaces:**
- Consumes: `payload.content.pairs`
- Produces: State for `unmatchedConcepts`, `unmatchedOptions`, `matches`, and `interactionState`.

- [ ] **Step 1: Rewrite imports, types, and state initialization**
...
git commit -m "feat: setup sequential state for MatchingExercise"
```

---

### Task 2: Implement UI and Sequential Interaction Flow

**Files:**
- Modify: `src/components/exercise/MatchingExercise.tsx`

**Interfaces:**
- Consumes: `unmatchedConcepts`, `unmatchedOptions`, interaction state
- Produces: `handleOptionPress` logic and the sequential UI layout.

- [ ] **Step 1: Add interaction logic and UI render**
...
```bash
git add src/components/exercise/MatchingExercise.tsx
git commit -m "feat: implement sequential matching layout and interactions"
```
