...
import { View, Text, TouchableOpacity, RefreshControl, SafeAreaView, FlatList } from 'react-native';
...
const jobSeekerPosts: JobSeekerPost[] = [
  ...
  },
  {
    ...
    createdAt: new Date(new Date().setDate(new Date().getDate() - 1)).toISOString(), // 1 day ago (NEW)
  ...
  },
  {
    ...
    createdAt: new Date(new Date().setDate(new Date().getDate() - 2)).toISOString(), // 2 days ago (NEW)
    category: 'Design'
  },
  {
    id: '4',
    ...
    interest: 'Senior Frontend Developer with expertise in React and Node.js',
    privacy: 'Public',
    skills: ['React', 'TypeScript', 'Node.js', 'MongoDB'],
  ...
  },
  {
    ...
    interest: 'Machine Learning Engineer with expertise in PyTorch and computer vision',
    privacy: 'Private',
...
const referrerPosts: ReferrerPost[] = [
  ...
  },
  {
    ...
    status: 'Active',
    expiresAt: Date.now() + 1000 * 60 * 60 * 24 * 10, // 10 days
    createdAt: new Date(new Date().setDate(new Date().getDate() - 7)).toISOString(), // 7 days ago
  },
  {
    id: '6',
    type: 'Referrer',
...
export default function HomeScreen() {
  ...
  const groupedJobSeekerPosts = useMemo(() => {
    // Group by category
    const groupByCategory = jobSeekerPosts.reduce<Record<string, JobSeekerPost[]>>(
      (groups, post) => {
  ...
  const groupedReferrerPosts = useMemo(() => {
    // Group by company
    const groupByCompany = referrerPosts.reduce<Record<string, ReferrerPost[]>>(
      (groups, post) => {
