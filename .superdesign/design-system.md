# Happy Mobile Exercise Design System
...
## Target redesign constraints

- Platform: iOS-first React Native exercise flow at approximately 390x844 logical points.
- Preserve the existing shared header, progress indicator, sticky Continue action, and Back action.
- Cormorant is reserved for the screen title. Geist handles every choice, label, explanation, and action.
- Main task must be obvious within one glance. Supporting help is collapsed by default.
- Emotions should scan quickly without ten equal-weight outlined pills.
- Cognitive patterns should use flat list rows and dividers rather than repeated rounded cards.
- AI help is entirely hidden until the learner explicitly opens “Need help spotting a pattern?”
- Never label output as “AI read” or “AI summary.” Phrase it as optional possible matches.
- No modal, blur backdrop, nested card, gradient, decorative assistant icon, or long clinical paragraph.
- A selected state must use check icon plus border/background change; never color alone.
